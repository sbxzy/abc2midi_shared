 ...
 * Wil Macaulay (wil@syndesis.com)
 ...
 Modified by sbxzy(email: sbxzy@foxmail.com) into a shared library, i.e. a dll file. You can compile it with mingw.
...
MYDLL_API int reverted(char* times, char* outtimes);
...
void event_init(in_filename, out_filename, do_harp, maxnotes_in)
...
*/
/*outbase = addstring(in_filename); //[RM] 2010-11-21 
 for (j = (int) strlen(outbase); j>0 ; j--)  {
  ...
  /* look for user-supplied output filename */
      outname = addstring(out_filename);
      userfilename = 1;
      if (xmatch == 0) {
        xmatch = -1;
      };
  ...
  ratio_standard = -1; /* [SS] 2016-01-02 */
  quiet  = -1;
  dotune = 0;
  parseroff();
  setup_chordnames();
...
char* outtimes;
{
  ...
  event_init(&filename, outtimes, 1, 500);
  filename = times;
  /* [SS] 2013-04-10 */
